...
start_jupyter_and_write_log(){
    ...
    ssh -S ${SSH_MASTER_SOCKET} -M $REMOTE 'bash --login -s' < $REMOTE_SCRIPT &> $JUPYTER_LOG &
...
echo 'CLICK ON THE "SHUT DOWN" BUTTON ON THE JUPYTER WEB INTERFACE.'
echo '        AND TERMINATE THIS PROCESS WITH CTRL+C.'
echo '      THEN YOU CAN CLOSE THE TERMINAL IF YOU WISH.'
echo '    BUT DON'T CLOSE THE TERMINAL UNTIL THAT IS DONE!'
echo
